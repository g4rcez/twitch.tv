...
const Input = (props: React.InputHTMLAttributes<HTMLInputElement>) => (
  <input
    {...props}
    className={`border p-2 border-slate-400 rounded-md ${
      props.className ?? ""
    }`}
  />
);
...
export default function Home() {
  ...
  return (
    <div className="w-screen flex flex-col justify-center items-center gap-4 h-screen bg-white text-black">
      {state === null ? null : (
        <nav>
      ...
      <form className="flex flex-col gap-4" onSubmit={onSubmit}>
        <Input data-testid="name" name="name" placeholder="Apelido" />
        <Input data-testid="url" name="url" placeholder="URL" />
        <button
          type="submit"
          data-testid="submit"
